          ...
          <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
            ...
            <main class="modal__content" id="modal-1-content">
                ...
                if(isset($_GET['edit-user']) && !empty($_GET['edit-user'])){
                    $edit = "SELECT `id`, `name`, `email`, `phone` FROM clients";
                    if($result = $DB->prepare($edit)){
                        if($result->num_rows > 0){
                            foreach($result as $row){
                    $_S = $row['id'];
                    $_SE['email'] = $row['email'];
                    $_SES['name'] = $row['name'];
                    $_SESS['phone'] = $row['phone'];
                            }
                            $_S = $DB->real_escape_string($_GET["id"]);
                            $_SE = $DB->real_escape_string($_GET["email"]);
                            $_SES = $DB->real_escape_string($_GET["name"]);
                            $_SESS = $DB->real_escape_string($_GET["phone"]);
                    if($_S > 0){
                    $edit = "UPDATE clients SET `name`='$_SES', email='$_SE', phone='$_SESS' WHERE id = '$_S'";
                    if($result = $DB->prepare($edit)){
                    // foreach($result as $row){
                    //     $name = $row['name'];
                    //     $email = $value['email'];
