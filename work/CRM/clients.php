...
<body>
    ...
    <div class="modal micromodal-slide" id="add-modal" aria-hidden="true">
      ...
      <div class="modal micromodal-slide <?php
                    if(isset($_GET['edit-user']) && !empty($_GET['edit-user'])){echo ' open';}
          ...
          <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
            ...
            <?php
                if(isset($_GET['edit-user']) && !empty($_GET['edit-user'])){
                    $id = $_GET['edit-user'];
                    echo $id;
                    $SE =  $DB->query("SELECT `id` FROM clients WHERE id='$id'")->fetchAll();
                    foreach($SE as $key2 => $value2){
                        $date = $DB->query("SELECT `name`, `email`, `phone` FROM clients WHERE id='$value2[id]'")->fetchAll();
                ...
                //    1. Записать ид пользователя
               // 2. по ид пользователя получаешь данные
            

            //    $DB->close();
                ?>
                <form action="api/clients/EditClient.php" method="POST" class="modal__form">
                    <div class="modal__form-group">
                        <label for="fullname">ФИО</label>
                        <input type="text" value="<?php echo $value['name'] ?>" id="fullname" name="fullname">
                    </div>
                    <div class="modal__form-group">
                        <label for="email">Почта</label>
                        <input type="email" value="<?php echo $value['email'] ?>" id="email" name="email">
                    </div>
                    <div class="modal__form-group">
                        <label for="phone">Телефон</label>
                        <input type="tel" value="<?php echo $value['phone']?>" id="phone" name="phone">
                    </div>
                    <div class="modal__form-actions">
